...
public class CameraRandom : MonoBehaviour
{
    ...
    private void ChangeCameraIndex()
    {
        if (!movingCamera)
        {
            if (Input.GetButtonDown("LeftBumper") || Input.GetAxis("LeftTrigger") != 0f)
            {
                cameraTrueIndex -= 1;
            ...
            else if (Input.GetButtonDown("RightBumper") || Input.GetAxis("RightTrigger") != 0f)
            {
                cameraTrueIndex += 1;
    ...
    private void RotateCamera()
    {
        ...
        else
        {
            ...
            else if (Input.GetAxis("RightJoyX") > 0f)
            {
                transform.RotateAround(centers[centerNumber].transform.position, new Vector3(0, 1, 0), -rotateSpeed * Time.deltaTime);
            }
        }
