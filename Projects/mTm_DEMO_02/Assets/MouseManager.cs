...
public class MouseManager : MonoBehaviour
{
    ...
    void SetCursor()
    {
        ...
        else
        {
            ...
            {
                Cursor.SetCursor(cursor01, new Vector2(0, 0), CursorMode.Auto);
            }
            else
            {
                if (!Input.GetMouseButtonDown(0) || hitInfo.collider == null)
                {
                    Cursor.SetCursor(cursor02, new Vector2(0, 0), CursorMode.Auto);
                }
                else
                {
                    Debug.Log("DianLe");
                    Cursor.SetCursor(cursor03, new Vector2(0, 0), CursorMode.Auto);
                    kaiGuan = true;
                }
            }
        }
